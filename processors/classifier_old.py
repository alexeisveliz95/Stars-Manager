...
def assign_category(repo):
    # Unificamos el texto para análisis
    text_to_analyze = f"{repo.name} {repo.description} {' '.join(repo.topics or [])}".lower()

    # Categorización por orden de importancia en CATEGORIES_DB
    for category, keywords in CATEGORIES_DB.items():
        if any(_matches_keyword(text_to_analyze, kw) for kw in keywords):
            return category
    return "Otros"
...
def calculate_score(stars_total, stars_today):
    ...
    # Pequeño bonus por ser Open Source activo
